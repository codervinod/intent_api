...
var clusterReferenceTypeKindPropEnum []interface{}

const (
	clusterReferenceKindCluster string = "cluster"
)

// prop value enum
func (m *ClusterReference) validateKindEnum(path, location string, value string) error {
	if clusterReferenceTypeKindPropEnum == nil {
		var res []string
		if err := json.Unmarshal([]byte(`["cluster"]`), &res); err != nil {
			return err
		}
		for _, v := range res {
			clusterReferenceTypeKindPropEnum = append(clusterReferenceTypeKindPropEnum, v)
		}
	}
	if err := validate.Enum(path, location, value, clusterReferenceTypeKindPropEnum); err != nil {
		return err
	}
	return nil
}
